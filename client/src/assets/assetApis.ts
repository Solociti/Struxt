...
export async function getAsset(projectId: string, uuid: string) {
  const response = await getApi<AssetApi>(
    ["/api/assets/model", projectId],
    { uuid }
  );
 ...
 */
export async function saveAssetContent(projectId: string, uuid: string, content: string) {
 ...
 * Create new asset
 */
export async function createNewAsset(projectId: string, values: { displayName: string, path: string }) {
